...
\abstract{The partial differential equations describing reacting flows can be notoriously difficult to resolve on a pragmatic scale and often require the use of high performance computing systems and/or accelerators. 
However, these systems face scaling issues such as latency or the fixed cost of communicating information between devices in the system. The swept rule is a technique designed to minimize these costs by obtaining a solution to unsteady equations at as many possible locations and times prior to communicating. 
In this study, we implemented and tested the swept rule in two dimensions on two sets of heterogeneous hardware. In this process, our solver showed a speedup range of 0.52--1.46 for the compressible Euler equations and 0.22--2.71 for the heat diffusion equation. 
We can conclude from this study that there is both potential for speedups and slowdowns when using swept rule and that care should be taken when designing such a solver to maximize benefits. It is our belief that these results can help make decisions to maximize these benefits and inform designs.}

% Keywords
\keyword{Latency; HPC; GPU; CPU; PDE} 
