...
% \revised{Compressible flow is a good example of an expensive numerical simulation because it involves fluid mechanics and heat transfer over massive areas of impact. Compressible flow can also be unsteady---so it changes over time.}{revTwo}
...
This study presents our implementation and testing of a two-dimensional heterogeneous solver for unsteady partial differential equations that employs a technique to help overcome the latency barrier---the swept rule \cite{alhubail_swept_2016-1}. In other words, the swept rule is a latency reduction technique that focuses on obtaining a solution to unsteady PDEs at as many possible locations and times prior to communicating with other computing nodes (ranks). In this article, we first discuss related work, distinguish our work from prior swept studies, and provide more motivation in Section~\ref{related-section}. Next, we describe implementation details, objectives, study parameters, design decisions, methods, and tests in Section~\ref{methods-section}. As expected, this is followed with results and conclusions in Sections~\ref{results-section} and~\ref{conclusions-section}.

\section{Related Work}
\label{related-section}
Surmounting the latency barrier has been approached in many ways; the most closely related to this study include prior swept rule studies which involve multiple dimensions and architectures but not the combination of the two \cite{alhubail_swept_2016-1 ,alhubail_swept_2016,magee_accelerating_2018,magee_applying_2020}. Parallel-in-time methods are also related to the swept rule but differ in the sense that they iteratively parallelize the temporal direction, while the swept rule minimizes communication \cite{gander_50_2015}. There are many examples of parallel-in-time methods which are all variations of the core concept \cite{falgout_parallel_2014,maday_adaptive_2020,wu_parareal_2018,emmett_toward_2012,minion_interweaving_2015,hahne_pymgrit_2020}. For example, Parareal is one of the more popular parallel-in-time methods; it works by iterating over a series of coarse and fine grids with initial guesses to parallelize the problem \cite{maday_adaptive_2020}. 
...
They concluded that in each case a number of integrations can be performed during the \former{latency}{revTwo} time of a communication. Their analysis showed that integration can be made faster by latency reduction and increasing computing power of the computing nodes \cite{alhubail_swept_2016-1}. 
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Process diagrams for the \Swept{} and \Standard{} solvers. \textbf{Squares}: pre and post processing. \textvf{Circles}: calculation local to a node for a given time step. \textbf{Diamonds}: communication between nodes.}
...
More specifcally, the swept algorithm works by solving as many spatial points of a time step as it can and repeating this for subsequent time steps until information is required from neighboring nodes or hardware. This leaves the solution in a state with several incomplete time steps. Communication is then necessary to continue and fill in the incomplete time steps. This process repeats in various forms until the entire solution space has been completed to a desired time level. We first demonstrate this in one dimension on two nodes with a three point stencil in Figures~\ref{fig:1d-swept-rule}. The black boxes in Figure~\ref{fig:1d-swept-rule} represents the block size which is a fixed parameter throughout the simulation---in this case it is 12. The blocksize is a metric used to describe decomposition on GPUs which we formally discuss in the next section. 
...
The process in one-dimension progresses Figure~\ref{fig:UpTriangle} 
...
\linenumbers
\switchcolumn
%\former{Our performance metric of choice for the swept rule was speedup, $S$, as a function of array size, block size, and share:
%\begin{equation}
%    S = \frac{R_{\textit{standard}}}{R_{\textit{swept}}} \;,
%\end{equation}
%where $R_i$ is the run time of simulation $i$.}{revTwo}
...
\end{paracol}
\nointerlineskip
%Make subfigure and have standard first
\begin{figure}[htbp]
    \widefigure
...
\revised{Figures~\ref{fig:tptNewHeatSwept} and~\ref{fig:tptNewHeatStandard} show the performance of the two solvers as time per timestep. As you }{revTwo}
...
\revised{Figures~\ref{fig:tptOldHeatSwept} and~\ref{fig:tptOldHeatStandard} show the performance of the two solvers as time per timestep. As you }{revTwo}
...
\revised{Figures~\ref{fig:tptNewEulerSwept} and~\ref{fig:tptNewEulerStandard} show the performance of the two solvers as time per timestep. As you }{revTwo}
...
\revised{Figures~\ref{fig:tptoldEulerSwept} and~\ref{fig:tptoldEulerStandard} show the performance of the two solvers as time per timestep. As you }{revTwo}
